...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  $existsql= "SELECT * from `users` where usename='$username' ";
  $existresult= mysqli_query($con, $existsql);
  $existNumRows= mysqli_num_rows($existresult);
  if($existNumRows > 0 ){
    echo '<div class="alert alert-warning alert-dismissible fade show" role="alert">
  <strong>Holy guacamole!</strong> user name already exist
  ...
  if ($password == $cpassword) {
    ...
    $sql= "INSERT into `users` (`usename`, `password`) values ('$username', '$hash')";
    $result= mysqli_query($con, $sql);
    if ($result) {
      echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
      <strong>Holy guacamole!</strong> Thank you for signup
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>';
    }
    ...
    echo '<div class="alert alert-warning alert-dismissible fade show" role="alert">
    <strong>Holy guacamole!</strong> Password is misatch
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  </div>';
  }
  ...
  <body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      ...
      <div class="container my-4">
          <h1 class="text-center">Signup on our site</h1>
    <form style="display: flex;
    align-items: center;
    justify-content: center;
    flex-direction: column;" action="/loginsystem/signup.php" method="POST">
        ...
        <div class="mb-3 col-md-6">
            <label for="exampleInputPassword1" class="form-label">Password</label>
            <input type="password" class="form-control" id="cPassword1" name="cpassword" placeholder="confirm password">
          </div>
